---
product: journey orchestration
user-guide-title: Tutoriais do Journey Orchestration
user-guide-description: Saiba como configurar, criar, publicar e analisar uma jornada.
user-guide-url: https://docs.adobe.com/content/help/en/journey-orchestration-learn/tutorials/overview.html
solution-title: Experience League
solution-hub-url: https://experienceleague.adobe.com/?tag=Journey+Orchestration#recommended/solutions/journey-orchestration
tutorials-title: Tutorials
tutorials-url: https://docs.adobe.com/content/help/en/journey-orchestration-learn/tutorials/overview.html
getting-started-title: Help Center
getting-started-url: https://docs.adobe.com/content/help/en/journeys/using/journey-orchestration-home.html
auto-video-transcripts: true
source-git-commit: 5781e279d59d181619345f2873302479b6c28a84
workflow-type: tm+mt
source-wordcount: '73'
ht-degree: 100%

---


# Tutoriais do Journey Orchestration {#tutorials}

+ [Tutoriais do Journey Orchestration](/help/overview.md)
+ [Conheça o Journey Orchestration](/help/understanding-journey-orchestration.md)
+ [Configurar eventos de transmissão](/help/configuring-journey-orchestration/configure-streaming-events.md)
+ [Uso de eventos de qualificação de segmento](/help/using-segment-qualification-events.md)
+ [Configurar fontes de dados](/help/configuring-journey-orchestration/configure-data-sources.md)
+ [Configurar ações](/help/configuring-journey-orchestration/configure-actions.md)
+ Criar uma jornada {#building-a-journey}
   + [Criar uma jornada](/help/building-a-journey/creating-a-journey.md)
   + [Mudar para outra jornada](/help/building-a-journey/jumping-to-another-journey.md)
   + [Atualizar ação do perfil](/help/building-a-journey/update-profile-action.md)
+ [Validar e publicar uma jornada](/help/validate-and-publish-a-journey.md)
+ [Analisar uma jornada através das ferramentas de relatórios](/help/analyze-a-journey-via-reporting-tools.md)
+ [Eventos de etapa da jornada dos relatórios para Adobe Experience Platform](/help/reporting-step-events-to-adobe-experience-platform.md)
+ [Como obter ajuda com o Journey Orchestration](/help/how-to-find-help-with-journey-orchestration.md)
